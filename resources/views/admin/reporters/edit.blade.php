...
@section('breadcrumbs')
    ...
    <span class="breadcrumb-separator">›</span>
    <a href="{{ route('admin.reporters.show', $reporter) }}" class="breadcrumb-item">{{ $reporter->name }}</a>
    <span class="breadcrumb-separator">›</span>
...
<div class="uf-layout">
        ...
        <div class="uf-card-body">
            <div class="uf-row">
                <div class="uf-group">
                    ...
                    <input class="uf-input @error('name') uf-input-error @enderror" type="text" id="name" name="name" value="{{ old('name', $reporter->name) }}" required placeholder="John Doe">
                    @error('name')<p class="uf-error">{{ $message }}</p>@enderror
                </div>
                <div class="uf-group">
                    <label class="uf-label" for="email">Email Address <span class="uf-req">*</span></label>
                    <input class="uf-input @error('email') uf-input-error @enderror" type="email" id="email" name="email" value="{{ old('email', $reporter->email) }}" required>
            ...
            <div class="uf-group" style="margin-bottom:.75rem;">
                ...
                <input class="uf-input @error('xelenic_id') uf-input-error @enderror" type="text" id="xelenic_id" name="xelenic_id" value="{{ old('xelenic_id', $reporter->xelenic_id) }}" placeholder="e.g. XEL-001">
    ...
    <div class="uf-card">
        ...
        <div class="uf-card-footer">
            <p style="font-size:.72rem;color:#9ca3af;margin:0;">Ensure the <strong>reporter</strong> role remains assigned so this user retains reporter access.</p>
        </div>
    </div>
